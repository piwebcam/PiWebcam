<?php 
	...
	if(count($_REQUEST) > 0) {
		...
		if ($_REQUEST["action"] === "system") {
			if (array_key_exists("NAME",$_REQUEST)) run("set_name '".$_REQUEST["NAME"]."'");
			if (array_key_exists("PASSWORD",$_REQUEST)) run("set_password '".$_REQUEST["PASSWORD"]."'");
			if (array_key_exists("TIMEZONE",$_REQUEST)) run("set_timezone '".$_REQUEST["TIMEZONE"]."'");
			if (array_key_exists("COUNTRY_CODE",$_REQUEST)) run("set_country_code '".$_REQUEST["COUNTRY_CODE"]."'");
		...
		if ($_REQUEST["action"] === "upgrade") {
			if ($_FILES["file"]["error"] === 0) {
				...
				load_config();
				array_push($message["success"],"Device updated successfully");
			} else array_push($message["warning"],"Invalid firmware provided");
		}
		// reboot form
	...
	generate_modal("reboot_modal","The device will be rebooted. Please allow 30-60 seconds before reconnecting.","reboot_button","reboot_form");
	generate_modal("factory_reset_modal","By restoring factory defaults all the current settings will be lost and the camera pictures/movies deleted.<br><br>This operation is irreversible.<br><br>The system will automatically reboot after restoring to its original state.<br>Please allow 30-60 seconds before reconnecting.","factory_reset_button","factory_reset_form");
	generate_modal("upgrade_modal","The device is about to be upgraded.<br><br>The system might reboot to complete the process and process the update.<br>Please allow 30-60 seconds before reconnecting.","upgrade_button","upgrade_form");
										...
										<h3>Device Settings</h3>
											...
											<input type="hidden" name="action" value="system">
												<div class="form-group">
													<label>Name</label>
													<input class="form-control" name="NAME" value="<?php print $config["NAME"] ?>">
													<p class="help-block">The name of this device</p>
												</div>
												<div class="form-group">
													<label>Password</label>
													<input class="form-control" name="PASSWORD" type="password" value="<?php print $config["PASSWORD"] ?>">
													...
													<select name="TIMEZONE" class="form-control">		
														<?php
															...
															foreach ($timezones as $index => $timezone) {
																...
																if ($config["TIMEZONE"] === $timezone) print " selected"; 
													...
													<select name="COUNTRY_CODE" class="form-control">
														<?php
															...
															foreach ($countries as $index => $country) {
																...
																if ($config["COUNTRY_CODE"] === $country) print " selected"; 
